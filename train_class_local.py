...
model.back2.requires_grad_ = (
    False  # I think this should have no grad, maybe later for fine tuning
)
print("model", model)
...
def training(model, dataloader, optimizer, criterion_class, criterion_reg):
    ...
    for epoch in range(epochs):
        for batch_idx, data in enumerate(dataloader):
            ...
            if (
                animal == True
            ):  # There is an animal (I know the ==True can be ommited, but like to add it for clarity)
            ...
            correct, average_correct = classification_info(
                total, output_class, Y_class, correct
            )
